...
angular.module("bandcommanderApp")
  .controller('StatusController', ["$scope",
    ...
    function ($scope, $routeParams, Page, Toolbar,
      ...
      else if ($routeParams.statusType == "deleted") {
        data.status = "💀 Data deleted";
        data.class = "status-deleted"
        $timeout(function () {
          $location.path('/');
        }, 5000);
      }
...
angular.module("bandcommanderApp")
  .controller('SingleEventController', ["$scope", "$http",
    '$routeParams',
    'Event', 'Page', 'Toolbar', 'User', "$location","Angularytics",
...
function ($scope, $http, $routeParams, Event, Page, Toolbar, User,
  ...
  $scope.deleteEvent = function () {
    var r = confirm("Sicher?");
      if (r == true) {
        Event.delete($routeParams.eventID);
        $location.path('/status/deleted');
        } else {
      }
